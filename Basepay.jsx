...
// ═══════════════════════════════════════════════════════════════
//  CONFIGURATION — À remplacer après déploiement sur Base Sepolia
// ═══════════════════════════════════════════════════════════════
const CONFIG = {
  ...
  // ⚠️ REMPLACER ces adresses après déploiement avec Remix/Hardhat
  FUSDC_ADDRESS: "0x0000000000000000000000000000000000000000",
  INVOICE_ADDRESS: "0x0000000000000000000000000000000000000000",
  TOKEN_DECIMALS: 6,
};

// ═══════════════════════════════════════════════════════════════
//  ABI minimaux (seules les fonctions utilisées)
// ═══════════════════════════════════════════════════════════════
const ERC20_ABI = [
...
// ═══════════════════════════════════════════════════════════════
//  ETHERS-LITE: minimal Web3 helpers (no external dependency)
//  Uses window.ethereum (MetaMask) JSON-RPC directly
// ═══════════════════════════════════════════════════════════════
...
const encodeFunctionCall = (sig, params = []) => {
  // Simple keccak256 selector via Web Crypto (first 4 bytes)
  // We'll use pre-computed selectors for our known functions
  const SELECTORS = {
    ...
    "decimals()": "313ce567",
    "symbol()": "95d89b41",
    "createInvoice(address,uint256,string)": "placeholder",
    "payInvoice(uint256)": "placeholder",
    "getInvoice(uint256)": "placeholder",
    "nextInvoiceId()": "placeholder",
  };
  const selector = SELECTORS[sig];
  if (!selector || selector === "placeholder") return null;
...
// ═══════════════════════════════════════════════════════════════
//  DEMO INVOICES (simulated — in real app, these come from chain)
// ═══════════════════════════════════════════════════════════════
const DEMO_INVOICES = [
  {
    id: 0,
...
const Btn = ({ children, onClick, variant = "primary", disabled, loading, style: sx, ...rest }) => {
  ...
  const base = {
    ...
    background: isPrimary
      ? hov ? "#0046dd" : palette.blue
      : hov ? palette.surfaceAlt : "transparent",
  ...
  return (
    <button
      style={base} onClick={onClick} disabled={disabled || loading}
...
const NetworkBadge = () => (
  <div style={{
    ...
    <span style={{
      width: 7, height: 7, borderRadius: "50%", background: palette.cyan,
...
const AiChat = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "Erreur de connexion à l'IA. Vérifie ta connexion." },
      ]);
    }
    setLoading(false);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 600, fontFamily: fontSans, color: palette.text }}>
              Assistant BasePay
            </div>
            <div style={{ fontSize: 10, color: palette.cyan, fontFamily: font }}>
      ...
      {/* Input */}
      <div style={{
        padding: 12, borderTop: `1px solid ${palette.border}`,
        display: "flex", gap: 8,
      }}>
        <input
          value={input}
        ...
        <div key={i} style={{ display: "flex", alignItems: "center" }}>
          <div style={{
            ...
            <span style={{
              ...
              color: i <= current ? palette.text : palette.textMuted,
          ...
          {i < steps.length - 1 && (
            <div style={{
              ...
              marginBottom: 20,
              background: i < current ? palette.blue : palette.border,
...
export default function BasePay() {
  ...
  const [step, setStep] = useState(0); // 0=select, 1=approve, 2=pay, 3=receipt
  ...
  const connect = async () => {
    if (!hasMetaMask) {
      setError("Installe MetaMask pour continuer !");
      return;
    }
    try {
      setError(null);
      ...
      setError("Connexion refusée. Réessaie.");
    }
  };
  ...
  const handleApprove = async () => {
    ...
    setStep(1);
    try {
      // In production: call token.approve(invoiceContract, amount)
      // Demo simulation with delay
      await sleep(2000);
      setStep(2);
    } catch (err) {
      setError("Approve échoué : " + (err.message || err));
    }
    setLoading(false);
  };
  ...
  const handlePay = async () => {
    ...
    setError(null);
    try {
      // In production: call invoiceContract.payInvoice(invoiceId)
      await sleep(3000);
      ...
      setInvoices((prev) =>
        prev.map((inv) =>
          inv.id === selected.id ? { ...inv, status: 1, paidAt: Date.now() } : inv
        )
      );
      setSelected((prev) => ({ ...prev, status: 1, paidAt: Date.now() }));
      setStep(3);
      setBalance((prev) => {
        const remaining = parseFloat(prev) - parseFloat(selected.amount);
        return remaining.toFixed(2);
      });
    } catch (err) {
      setError("Paiement échoué : " + (err.message || err));
    }
    setLoading(false);
  };
  ...
  const containerStyle = {
    ...
    fontFamily: fontSans, position: "relative", zIndex: 1,
  };
    ...
    <div style={containerStyle}>
      ...
      <div style={innerStyle}>
        ...
        <div style={{
          ...
          marginBottom: 40, animation: "fadeIn 0.5s ease",
        }}>
          <div>
            <div style={{
              fontSize: 28, fontWeight: 700, letterSpacing: "-0.02em",
              background: `linear-gradient(135deg, ${palette.white}, ${palette.cyan})`,
              WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
            }}>
              BasePay
            </div>
            <div style={{
              fontSize: 12, color: palette.textDim, fontFamily: font,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            {account ? (
              <div style={{
                ...
                {balance && (
                  <span style={{
                    fontSize: 11, color: palette.cyan, fontFamily: font,
                    padding: "2px 8px", background: palette.blueDim, borderRadius: 6,
                  }}>
                    {balance} fUSDC
                  </span>
                )}
        ...
        {error && (
          <div style={{
            ...
            background: palette.redDim, border: `1px solid ${palette.red}33`,
            color: palette.red, fontSize: 13, fontFamily: fontSans,
            animation: "fadeIn 0.3s ease",
          }}>
            ⚠️ {error}
          </div>
        ...
        {!account && (
          <Card style={{
            ...
            <h2 style={{
              ...
              color: palette.white, margin: "0 0 8px",
            }}>
              Connecte ton wallet
            </h2>
            <p style={{
              fontSize: 14, color: palette.textDim, marginBottom: 28,
              lineHeight: 1.6, maxWidth: 400, margin: "0 auto 28px",
            }}>
              Pour payer une facture en fUSDC sur Base Sepolia,
              connecte MetaMask. Le réseau sera ajouté automatiquement.
            </p>
            <Btn onClick={connect} style={{ padding: "14px 32px", fontSize: 15 }}>
              🦊 Connecter MetaMask
            </Btn>
          </Card>
        )}

        {/* Connected — Invoice List */}
        {account && !selected && (
          <div style={{ animation: "fadeIn 0.5s ease" }}>
            <div style={{
              ...
              <span style={{
                fontSize: 12, fontFamily: font, color: palette.textMuted,
              }}>
                {invoices.filter((i) => i.status === 0).length} en attente
              </span>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {invoices.map((inv, i) => (
                <Card
                  key={inv.id}
                  onClick={() => inv.status === 0 ? selectInvoice(inv) : null}
                ...
                >
                  <div style={{
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{
                        ...
                        <span style={{
                          fontSize: 11, fontFamily: font, color: palette.textMuted,
                          padding: "2px 8px", background: palette.surfaceAlt,
                      ...
                      <div style={{
                        fontSize: 15, fontWeight: 600, color: palette.white, marginBottom: 4,
                      }}>
                        {inv.description}
                      </div>
                      <div style={{ fontSize: 12, color: palette.textMuted, fontFamily: font }}>
                        Échéance : {inv.dueDate} · Émetteur : {inv.issuer}
                      </div>
                    </div>
                  ...
                  {inv.status === 0 && (
                    <div style={{
                      ...
                      <span style={{
                        fontSize: 12, color: palette.blue, fontWeight: 500,
        ...
        {/* Selected Invoice — Payment Flow */}
        {account && selected && (
          <div style={{ animation: "slideUp 0.4s ease" }}>
            {/* Back button */}
            <div
              onClick={step === 3 ? reset : () => { setSelected(null); setStep(0); }}
              style={{
                display: "inline-flex", alignItems: "center", gap: 6,
                fontSize: 13, color: palette.textDim, cursor: "pointer",
                marginBottom: 24, fontFamily: fontSans,
              }}
            >
              ← Retour aux factures
            </div>
            ...
            <Card glow={step < 3} style={{ marginBottom: 20 }}>
              <div style={{
                ...
                <div>
                  ...
                  <h3 style={{
                    fontSize: 18, fontWeight: 600, margin: "10px 0 4px",
                    color: palette.white,
                  }}>
                    {selected.description}
                  </h3>
                  <div style={{ fontSize: 12, color: palette.textMuted, fontFamily: font }}>
                    Émetteur : {selected.issuer}
                  </div>
                </div>
              ...
              {/* Amount */}
              <div style={{
                padding: 20, borderRadius: 12, background: palette.surfaceAlt,
                border: `1px solid ${palette.border}`, textAlign: "center",
                marginBottom: 20,
              }}>
                <div style={{ fontSize: 11, color: palette.textMuted, fontFamily: font, marginBottom: 4 }}>
                  MONTANT À PAYER
                </div>
                <div style={{
                  ...
                  {selected.amount}
                  <span style={{ fontSize: 16, color: palette.cyan, marginLeft: 8 }}>fUSDC</span>
                </div>
                <div style={{ fontSize: 11, color: palette.textMuted, fontFamily: font, marginTop: 4 }}>
                  sur Base Sepolia · Token ERC-20
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
                {[
                  ...
                  ["Token", "fUSDC (Fake USDC)"],
                  ["Échéance", selected.dueDate],
                  ["Gas estimé", "~0.000002 ETH"],
                ].map(([k, v]) => (
                  <div key={k} style={{
                    display: "flex", justifyContent: "space-between",
              ...
              {/* Action buttons */}
              {step === 0 && (
                <Btn onClick={handleApprove} loading={loading} style={{ width: "100%" }}>
                  🔑 Étape 1 : Autoriser le contrat (Approve)
                </Btn>
              )}
              {step === 1 && (
                <div style={{ textAlign: "center" }}>
                  <div style={{
                    ...
                    border: `3px solid ${palette.border}`, borderTopColor: palette.blue,
                    borderRadius: "50%", animation: "spin 0.8s linear infinite",
                  }} />
                  <div style={{ fontSize: 13, color: palette.textDim }}>
                    Approbation en cours... Confirme dans MetaMask
                  </div>
                </div>
              )}
              {step === 2 && (
                <Btn onClick={handlePay} loading={loading} style={{ width: "100%" }}>
                  💸 Étape 2 : Payer la facture
                </Btn>
              )}
            </Card>
            ...
            {step === 3 && txHash && (
              <Card style={{
                ...
                background: `linear-gradient(135deg, ${palette.surface}, ${palette.greenDim})`,
                border: `1px solid ${palette.green}33`,
              }}>
                <div style={{ textAlign: "center", marginBottom: 20 }}>
                  <div style={{
                    width: 56, height: 56, borderRadius: "50%",
                    background: palette.greenDim, margin: "0 auto 12px",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 28, border: `2px solid ${palette.green}44`,
                  }}>
                    ✅
                  </div>
                  <h3 style={{
                    fontSize: 20, fontWeight: 700, color: palette.green, margin: "0 0 4px",
                  }}>
                    Paiement confirmé !
                  </h3>
                  <p style={{ fontSize: 13, color: palette.textDim, margin: 0 }}>
                    La facture a été réglée on-chain
                  </p>
                </div>

                <div style={{
                  padding: 16, borderRadius: 12, background: palette.surface,
                  border: `1px solid ${palette.border}`,
                  display: "flex", flexDirection: "column", gap: 10,
                }}>
                  <div style={{ fontSize: 11, fontFamily: font, color: palette.cyan, fontWeight: 600, letterSpacing: "0.06em" }}>
                    REÇU DE PAIEMENT
                  </div>
                  {[
                    ["Référence", selected.ref],
                    ["Montant", `${selected.amount} fUSDC`],
                    ["Payeur", shortenAddress(account)],
                    ["Date", new Date(paidTimestamp).toLocaleString("fr-FR")],
                    ["Statut", "Confirmé ✓"],
                  ].map(([k, v]) => (
                    <div key={k} style={{
                      display: "flex", justifyContent: "space-between",
                  ...
                  <div style={{
                    marginTop: 8, paddingTop: 10, borderTop: `1px solid ${palette.border}`,
                  }}>
                    <div style={{ fontSize: 11, color: palette.textMuted, fontFamily: font, marginBottom: 4 }}>
                      Transaction Hash
                    </div>
                    <a
                      ...
                      style={{
                        fontSize: 11, fontFamily: font, color: palette.blue,
                ...
                <a
                  href={`https://warpcast.com/~/compose?text=${encodeURIComponent(
                    `✅ Facture ${selected.ref} payée on-chain !\n\n💰 ${selected.amount} fUSDC sur Base Sepolia\n📄 ${selected.description}\n🔗 ${CONFIG.EXPLORER}/tx/${txHash}\n\nPowered by BasePay — paiement de factures en stablecoin 🚀\n\n#BasePay #Base #Web3 #Stablecoin #OnchainPayment`
                  )}`}
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                >
                  <div
                    ...
                    onMouseEnter={(e) => {
                      ...
                      e.currentTarget.style.boxShadow = "0 6px 24px #8B5CF655";
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.transform = "translateY(0)";
                      e.currentTarget.style.boxShadow = "0 4px 16px #8B5CF633";
                    }}
                  >
                ...
                <Btn onClick={reset} variant="secondary" style={{ width: "100%", marginTop: 8 }}>
                  ← Retour aux factures
                </Btn>
              </Card>
            )}
        ...
        {/* Footer */}
        <div style={{
          marginTop: 48, paddingTop: 20, borderTop: `1px solid ${palette.border}`,
          textAlign: "center", fontSize: 11, color: palette.textMuted,
